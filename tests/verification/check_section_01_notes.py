...
# -*- coding: utf-8 -*-
"""
Script para extraer y mostrar las notas de la sección 1 del archivo Excel.
"""
import pandas as pd
import re
import os
...
# Variable para almacenar las notas de la sección 1
section_1_notes = []
in_section_1 = False
...
# Abrir el archivo Excel y buscar la Sección 1
if os.path.exists(excel_path):
    ...
    if sheet_names:
        ...
        # Iteramos por las filas para encontrar la sección 1
        for i, row in df.iterrows():
            ...
            # Verificamos si encontramos la sección 1
            section_match = re.search(r'Sección\s+I\b|SECCION\s+I\b|SECCIÓN\s+I\b', row_text, re.IGNORECASE)
            if section_match:
                print(f"Encontrada Sección I en fila {i+1}: {row_text}")
                in_section_1 = True
            ...
            # Si estamos en la sección 1, verificamos si son notas
            if in_section_1:
                ...
                if re.search(r'Notas\.?|NOTAS\.?', row_text, re.IGNORECASE):
                    ...
                    section_1_notes.append(row_text)
                    continue
...
complete_notes = section_title + "\n" + "\n".join(section_1_notes)

# Mostrar las notas obtenidas
print("\nNotas de la Sección 1 obtenidas del Excel:")
print("-" * 80)
...
with app.app_context():
    # Obtenemos la nota de la sección 1
    section_note = SectionNote.query.filter_by(section_number='01').first()
    if section_note:
        print("-" * 80)
        ...
        print("No se encontraron notas para la sección 1 en la base de datos.")
